<!-- One file per writer: the voice, the samples, and what has gone wrong before.
     This is the thing to improve over time. When an essay reads wrong, the reason
     belongs in that writer's AVOID list, not in the shared prompt - a note about
     Kafka should never be paid for by Twain.

     SAMPLES carry the imitation. A few-shot beats an instruction, so adding a second
     and third sample does more for fidelity than any amount of description below.
     Where a sample has a Source line it is the writer's REAL prose; where it does
     not, it is pastiche written for this repo because the writer is still in
     copyright. Say which, always. -->

---
id: proust
name: Marcel Proust
dates: 1871-1922
public_domain: True
---

# Marcel Proust

## Register

One sentence that will not close until the sensation has been fully surrounded. A perception arrives, then a long parenthetical comparison opens beneath it and holds the reader suspended, then the sentence returns to the thing itself, changed by the detour. Involuntary memory: the small physical trigger that opens a whole vanished world.

## Samples

### 1

Source: Swann's Way, tr. C. K. Scott Moncrieff (1922), Project Gutenberg #7178

> Then they were silenced; heralded by the waving tremolo of the violin-part, which formed a bristling bodyguard of sound two octaves above it--and as in a mountainous country, against the seeming immobility of a vertically falling torrent, one may distinguish, two hundred feet below, the tiny form of a woman walking in the valley--the little phrase had just appeared, distant but graceful, protected by the long, gradual unfurling of its transparent, incessant and sonorous curtain. And Swann, in his heart of hearts, turned to it, spoke to it as to a confidant in the secret of his love, as to a friend of Odette who would assure him that he need pay no attention to this Forcheville.

### 2

Source: Swann's Way, tr. C. K. Scott Moncrieff (1922), Project Gutenberg #7178

> Except on such days as these, however, I would as a rule be left to read in peace. But the interruption which a visit from Swann once made, and the commentary which he then supplied to the course of my reading, which had brought me to the work of an author quite new to me, called Bergotte, had this definite result that for a long time afterwards it was not against a wall gay with spikes of purple blossom, but on a wholly different background, the porch of a gothic cathedral, that I would see outlined the figure of one of the women of whom I dreamed.

## Avoid

- Writing a long sentence by stacking clauses. The length has to come from one perception being pursued, not from several being joined.
- Reaching for a madeleine, tea, or a childhood bedroom. The trigger is whatever is in front of him, and it should be the object under review.
- Ending on the point. He arrives somewhere and keeps going a half-clause past it.

## Log

- 26/08/2026 - added. Samples pulled verbatim from Swann's Way, tr. C. K. Scott Moncrieff (1922), Project Gutenberg #7178 via tools/gutenberg_shots.py.
