<!-- One file per writer: the voice, the samples, and what has gone wrong before.
     SAMPLES carry the imitation. Where a sample has a Source line it is the writer's
     REAL prose; where it does not, it is pastiche written for this repo because the
     writer is still in copyright. Say which, always. -->

---
id: montaigne
name: Michel de Montaigne
sort: Montaigne
dates: 1533-1592
public_domain: True
---

# Michel de Montaigne

## Register

Thinks aloud and changes his mind while doing it. Long accumulating sentences held together by semicolons and colons. Starts from himself - what he does, what he cannot manage, what he has noticed about his own habits - and reaches the general claim only by way of the particular. Admits ignorance readily and treats the admission as the argument. Never arrives anywhere final.

## Samples

### 1

Source: Essays, "Of Presumption", trans. Charles Cotton, Project Gutenberg #3600

> I was present one day at Barleduc, when King Francis II, for a memorial of Rene, king of Sicily, was presented with a portrait he had drawn of himself: why is it not in like manner lawful for every one to draw himself with a pen, as he did with a crayon? I will not, therefore, omit this blemish though very unfit to be published, which is irresolution; a very great effect and very incommodious in the negotiations of the affairs of the world; in doubtful enterprises, I know not which to choose.

### 2

Source: Essays, trans. Charles Cotton, Project Gutenberg #3600

> These are all dreams and fanatic follies. Why will not nature please for once to lay open her bosom to us, and plainly discover to us the means and conduct of her movements, and prepare our eyes to see them? Good God, what abuse, what mistakes should we discover in our poor science! I am mistaken if that weak knowledge of ours holds any one thing as it really is, and I shall depart hence more ignorant of all other things than my own ignorance.

## Avoid

- Reaching a firm verdict. He circles, qualifies and leaves it open; a tidy conclusion is the wrong shape for him even when the piece needs a score attached to it.
- Quoting the ancients by name in every second sentence. He does it constantly in the real essays and it reads as padding here.

- The phrase is too direct and assertive, a reader of Montaigne would expect a more tentative and exploratory tone, such as 'I find myself wondering what profit a man might find' Seen in a published essay: "I know not what profit a man finds in startling his own soul out of its proper rest" (found 2026-09-02).
- The use of 'we' is too absolute, Montaigne would likely use 'I' to start and perhaps later generalize, as in 'I mock the slave, and yet I place upon my own nightstand' Seen in a published essay: "We mock the slave who moves at the sound of his master's bell, while we place upon our own nightstands a tiny executioner of our ease" (found 2026-09-02).
- The statement is too definitive, Montaigne would likely qualify it with more uncertainty, such as 'where I sometimes suspect I may be wiser' Seen in a published essay: "where I think I am often wiser than in my full soberness" (found 2026-09-02).
- A reader of Michel de Montaigne would have expected 'I' instead of 'we' to start the sentence, allowing for later generalization. Seen in a published essay: "so prompt are we to pardon in ourselves the very inclinations we condemn in others" (found 2026-09-03).
- A reader of Michel de Montaigne would have expected 'I' instead of 'we' to start the sentence, allowing for later generalization. Seen in a published essay: "We are so built that our tiny comforts must ever be bought at the expense of our neighbour's misery" (found 2026-09-03).
- A reader of Michel de Montaigne would have expected 'I' instead of 'we' to start the sentence, allowing for later generalization. Seen in a published essay: "We imagine we master nature by commanding hot water to rain at our pleasure" (found 2026-09-10).
## Log

- 25/08/2026 - added. Two real passages from the Cotton translation.
