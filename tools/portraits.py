...
# FIVE OF THESE WERE NOT DRAWN BY THIS TOOL. Adams, Bourdain, Child, Franklin and
# Tolstoy were generated by hand through ChatGPT on 28/08/2026, with three existing
# portraits attached as REFERENCE IMAGES and the LOOK and STYLE strings below pasted as
# the prompt. Cloudflare's window was spent and the alternatives could not match the
# hand; the prompts are in the session record and reproducible from these same strings.
#
# They came out better than Flux was managing, and for a reason worth keeping: the
# reference images. Franklin landed his spectacles, domed crown and shoulder-length hair
# in ONE draw, where Nietzsche took three rounds and Proust never got its moustache.
# `--force` would overwrite all five with Flux versions - don't, without looking first.
LOOK = {
