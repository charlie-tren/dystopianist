...
# NONE OF THESE WERE DRAWN BY THIS TOOL ANY MORE. All 23 portraits were replaced on
# 28/08/2026 with engravings generated from PHOTOGRAPHS of each writer, by hand through
# ChatGPT, then put through this file's own greyscale, 9% inset crop and 512px resize.
#
# That was the TODO item, arriving the same day it was written, and the result settles
# the argument the LOOK strings below kept losing. Proust has the flat moustache his
# line asks for and six Flux draws refused. Thoreau has the chin curtain. Nietzsche's
# hair is right. Austen and Dickinson are plain rather than doe-eyed. A photograph does
# in one step what no amount of wording achieved, because likeness stops being
# something the prompt has to argue for.
#
# The LOOK strings are KEPT, not deleted: tools/portrait-from-photo.md passes them
# alongside the photograph, and they remain the fallback if this tool is ever used
# again. But `--force` would now replace a set of real likenesses with Flux
# approximations - do not, without looking at both first.
LOOK = {
