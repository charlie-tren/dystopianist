...
written after them. Run over the 66 already published, they reject twelve - and all
...
def main() -> int:
    ...
    backlog = [e for e in entries
               if e["thinker"] in roster and faults(e, roster[e["thinker"]])]
    print(f"{len(backlog)} of {len(entries)} published essays fail today's gates")
    for e in backlog:
        print(f'  {(e.get("provider") or "?"):<11}{e["thinker"]:<11}'
              f'{e["object"][:24]:<25} {"; ".join(faults(e, roster[e["thinker"]]))}')
    if args.list:
        return 0
    print()
    ...
    print(f"\n{done} rewritten, {left} still failing"
    ...
    if left and not done and os.environ.get("GITHUB_ACTIONS"):
        print(f"::warning::revoice rewrote nothing this run; {left} essays still fail "
              "the gates. Expected on a day the free tier is spent. If this repeats "
